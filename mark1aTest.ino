...
//LiquidCrystal lcd(Q2_5, Q2_6, Q2_0, Q2_1, Q2_2, Q2_3); for M-Duino 50RRA
...
//////////////////////////////////////////////////////////////////
void complete() {
  // print finish message when program terminates
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(cycle);
  lcd.print("/");
...
void loop() {
  ...
  if (millis() % 10000 == 0) {
    for (int k = 0; k < 4; ++k) {
      if (commStat[k] != 0) {
        ...
        lcd.print("          ");
        errorMessage = "PSU " + String(k) + " error:" + String(commStat[k]) + " " + String(millis()/60000);
      }
    }
  }
  ...
  if (millis() % 5000 == 0) {
    ...
    int timeBegin = (millis()-startTime)/60000;
    lcd.print(timeBegin);
    lcd.print("min begin");
    lcd.print("              ");
  }
